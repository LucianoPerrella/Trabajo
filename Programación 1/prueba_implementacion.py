...
def LogicaConstruccion(lista_palabras,diccionario):
    ...
    while len(palabras_partida) != 6:
        ...
        if len(palabras_partida) ==5: #Sexta palabra - Vertical - Depende de como se formó la cuarta palabra
            if lista_direcciones[3].count("norte") > 0:#En caso de que la palabra N° 4 tenga dirección norte, buscaré palabras que coincidan con la primer letra
                indice_letra_a_buscar = 0
                letra_palabra = palabras_partida[3][indice_letra_a_buscar]
                siguiente_palabra, indice_coincidencia = random.choice(list(diccionario.get(letra_palabra).items()))
                while siguiente_palabra in palabras_partida and siguiente_palabra[-1] != letra_palabra:
                    siguiente_palabra, indice_coincidencia = random.choice(list(diccionario.get(letra_palabra).items()))
                lista_coincidencias.append([palabras_partida[3].rindex(letra_palabra), indice_coincidencia[0]])
                flag_direccion = "norte"
                lista_direcciones.append("vertical" + flag_direccion)
                palabras_partida.append(siguiente_palabra)
            else:
                ...
                letra_palabra = palabras_partida[3][indice_letra_a_buscar]
                siguiente_palabra, indice_coincidencia = random.choice(list(diccionario.get(letra_palabra).items()))
                while siguiente_palabra in palabras_partida and siguiente_palabra[0] != letra_palabra:
                    siguiente_palabra, indice_coincidencia = random.choice(list(diccionario.get(letra_palabra).items()))
                lista_coincidencias.append([palabras_partida[3].index(letra_palabra),indice_coincidencia[0]])
                flag_direccion = "sur"
                lista_direcciones.append("vertical" + flag_direccion)
                palabras_partida.append(siguiente_palabra)
...
def ConstruirTablero(tablero,lista_palabras,lista_coincidencias,direcciones):
    ...
    for i in range(len(lista_palabras)):
        ...
        for j in range(len(lista_palabras[i])):
            ...
            elif i == 5: #Sexta palabra - Vertical - Depende de  apalabra N 4
